...
function parseLinear(creative) {
    ...
    var skipoffset = single(creative.find('Linear')).attributes.skipoffset;

    if (duration) {
        duration = timestampToSeconds(duration.value);
    }
    if (skipoffset && skipoffset.includes('%') && duration) {
        skipoffset = Math.ceil(duration * (+skipoffset.replace('%', '') / 100));
    } else if (skipoffset) {
    skipoffset = timestampToSeconds(skipoffset);
    }
...
function parseNonLinear(creative) {
    ...
    return {
        ...
        ads: ads.map(function (ad) {
            var nonLinearClickThrough = single(ad.find('NonLinearClickThrough')).value || null;
            if (nonLinearClickThrough) {
                nonLinearClickThrough = nonLinearClickThrough.replace(/[\n\t\r]/g, "").trim();
            }
            var nonLinearClickTracking = single(ad.find('NonLinearClickTracking')).value || null;
            if (nonLinearClickTracking) {
                nonLinearClickTracking = nonLinearClickTracking.replace(/[\n\t\r]/g, "").trim();
            }
            ...
            return {
                ...
                clickThrough: nonLinearClickThrough,
                clickTracking: nonLinearClickTracking,
                parameters: adParameters.value
            };
        }),
        trackingEvents: events.map(function (event) {
...
module.exports = function pojoFromXML(xml) {
    ...
    if ($('Extensions')[0]) {
        var sf_extensions = $('Extensions')[0].children().filter(ext => {
            if ('type' in ext.attributes) return ext.attributes.type === "SmartFrame_optional_extensions";
        });

        if(sf_extensions.length > 0) {
            extensions = sf_extensions[0].children().map(function(extension) {
                var value = extension.value.replace(/[\n\t\r]/g, "").trim();
                return {
                    [extension.tag]: value
                }
            })
